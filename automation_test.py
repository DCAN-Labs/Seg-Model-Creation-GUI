...
def set_up_slurm_scripts(task_slurm: Path, all_slurm: Path):
    # Run before any step starts, it just copies over slurm scripts to a task folder within GUI code
    task_slurm.mkdir(exist_ok=True)
    for script in SCRIPTS:
        dest = task_slurm / script
        shutil.copyfile(all_slurm / script, dest)
    (task_slurm / "active_jobs.txt").write_text("")

def get_training_log_path(slurm_scripts_path, task_number, fold, job_id):
    # Returns the training output path
    return slurm_scripts_path / f"Train_{fold}_{task_number}_nnUNet-{job_id}.out"

def get_training_error_path(slurm_scripts_path, task_number, fold, job_id):
    # Returns the training error path
    return slurm_scripts_path / f"Train_{fold}_{task_number}_nnUNet-{job_id}.err"
...
### Min Maxes ###
def min_max(args, slurm_scripts_path, log_file_path, script_dir):
    ...
    submit_job(["sbatch", "-W", str(slurm_scripts_path / "create_min_maxes.sh"), args.synth_path, args.task_path, str(output_path)], log_file_path, "min_maxes")
    print("--- Min Maxes Created ---")

### SynthSeg Image Creation ###
def synthSeg_img(args, slurm_scripts_path, log_file_path, script_dir):
    ...
    submit_job([
        "sbatch", "-W",
        str(slurm_scripts_path / "SynthSeg_image_generation.sh"),
...
### Plan and Preprocess ###
def p_and_p(args, slurm_scripts_path, log_file_path, script_dir):
...
### Training Model ###
def model_training(args, slurm_scripts_path, log_file_path, script_dir):
    ...
    wait_fold_0_setup(
        get_training_log_path(slurm_scripts_path, args.task_number, 0, job_ids[0]),
        get_training_error_path(slurm_scripts_path, args.task_number, 0, job_ids[0])
    )
    ...
    while not all(complete):
        for i in range(5):
            ...
            err_file = get_training_error_path(slurm_scripts_path, args.task_number, i, job_ids[i])
...
def inference(args, slurm_scripts_path, log_file_path, script_dir):
...
if __name__ == '__main__':
    ...
    slurm_scripts_path = script_dir / "scripts" / "slurm_scripts" / args.task_number
    log_file_path = slurm_scripts_path / "active_jobs.txt"

    set_up_slurm_scripts(slurm_scripts_path, script_dir / "scripts" / "slurm_scripts")
    ...
    for step, should_run in zip(run_list, flags):
        if should_run:
            if step in [min_max, synthSeg_img, p_and_p, model_training, inference]: # These functions need extra arguments
                step(args, slurm_scripts_path, log_file_path, script_dir)
            else:
                step(args)
